...
mod test {
    ...
    #[bench]
    fn bench_bitmap_get_groups(b: &mut Bencher) {
        b.iter(|| {
            test::black_box(0x1BD837B).get_groups(5)
        });
    }

    #[bench]
    fn bench_bitmap_get_population(b: &mut Bencher) {
        b.iter(|| {
            let mut r = 0;
            for _ in 0..20 {
                r = test::black_box(0x1BD837B).get_population();
            }
            r
        });
    }
    ...
    #[bench]
    fn bench_bitmap_get_dimensions(b: &mut Bencher) {
        b.iter(|| {
            test::black_box(0x739C0).get_dimensions(5)
        });
    }

    #[bench]
    fn bench_bitmap_grow(b: &mut Bencher) {
        b.iter(|| {
            let mut r = 0;
            for _ in 0..10 {
                r = test::black_box(0x50140).grow(BOARD[5], 5);
            }
            r
        });
    }
