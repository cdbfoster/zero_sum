...
pub fn offset(a: &Vector, b: f32, c: &mut Vector) {
...
pub fn offset_assign(a: &mut Vector, b: f32) {
    use std::slice::from_raw_parts_mut as sm;

    if a.len() >= 16 {
        offset_assign_16s(a, b);
    }

    if a.len() % 16 >= 8 {
        unsafe { offset_assign_8(
            sm(a.buffer.offset((a.len() as isize / 8 - 1) * 8), 8), b,
        ) };
    }

    if a.len() % 8 >= 4 {
        unsafe { offset_assign_4(
            sm(a.buffer.offset((a.len() as isize / 4 - 1) * 4), 4), b,
        ) };
    }

    if a.len() % 4 >= 2 {
        unsafe { offset_assign_2(
...
#[cfg(feature = "with_simd")]
fn offset_16s(a: &[f32], b: f32, c: &mut [f32]) {
...
#[cfg(feature = "with_simd")]
fn offset_8(a: &[f32], b: f32, c: &mut [f32]) {
    let b_r = simd::x86::avx::f32x8::splat(b);
...
#[cfg(feature = "with_simd")]
fn offset_4(a: &[f32], b: f32, c: &mut [f32]) {
...
#[cfg(feature = "with_simd")]
fn offset_2(a: &[f32], b: f32, c: &mut [f32]) {
    let b_r = simd::f32x4::splat(b);
...
#[cfg(feature = "with_simd")]
fn offset_assign_16s(a: &mut[f32], b: f32) {
    let b_r = simd::x86::avx::f32x8::splat(b);
...
#[cfg(feature = "with_simd")]
fn offset_assign_8(a: &mut[f32], b: f32) {
    let b_r = simd::x86::avx::f32x8::splat(b);
...
#[cfg(feature = "with_simd")]
fn offset_assign_4(a: &mut[f32], b: f32) {
    let b_r = simd::f32x4::splat(b);
...
#[cfg(feature = "with_simd")]
fn offset_assign_2(a: &mut[f32], b: f32) {
    let b_r = simd::f32x4::splat(b);
...
mod tests {
    ...
    #[bench]
    fn bench_offset(bencher: &mut Bencher) {
    ...
    #[bench]
    fn bench_offset_assign(bencher: &mut Bencher) {
        let mut a = Vector::zeros(1000);
        for (i, x) in a.iter_mut().enumerate() {
            *x = (i % 16 + 1) as f32;
        }
