...
pub fn divide(a: &Vector, b: &Vector, c: &mut Vector) {
...
pub fn divide_assign(a: &mut Vector, b: &Vector) {
    debug_assert!(a.len() == b.len(), "Operand vectors are different lengths!");
    ...
    if a.len() >= 16 {
        divide_assign_16s(a, b);
    }

    if a.len() % 16 >= 8 {
        unsafe { divide_assign_8(
...
#[cfg(feature = "with_simd")]
fn divide_16s(a: &[f32], b: &[f32], c: &mut [f32]) {
    for i in 0..(a.len() / 16) {
...
#[cfg(feature = "with_simd")]
fn divide_8(a: &[f32], b: &[f32], c: &mut [f32]) {
...
#[cfg(feature = "with_simd")]
fn divide_4(a: &[f32], b: &[f32], c: &mut [f32]) {
...
#[cfg(feature = "with_simd")]
fn divide_2(a: &[f32], b: &[f32], c: &mut [f32]) {
...
#[cfg(feature = "with_simd")]
fn divide_assign_16s(a: &mut[f32], b: &[f32]) {
    for i in 0..(a.len() / 16) {
        let b_ra = simd::x86::avx::f32x8::load(b, i * 16 + 0);
        let b_rb = simd::x86::avx::f32x8::load(b, i * 16 + 8);
...
#[cfg(feature = "with_simd")]
fn divide_assign_8(a: &mut[f32], b: &[f32]) {
    let b_r = simd::x86::avx::f32x8::load(b, 0);
...
#[cfg(feature = "with_simd")]
fn divide_assign_4(a: &mut[f32], b: &[f32]) {
    let b_r = simd::f32x4::load(b, 0);
...
#[cfg(feature = "with_simd")]
fn divide_assign_2(a: &mut[f32], b: &[f32]) {
    let b_r = simd::f32x4::new(b[0], b[1], 0.0, 0.0);
...
mod tests {
    ...
    #[bench]
    fn bench_divide_assign(bencher: &mut Bencher) {
        let mut a = Vector::zeros(1000);
        for (i, x) in a.iter_mut().enumerate() {
            *x = (i % 16 + 1) as f32;
        }
